...
//JUI Dialog
//-------------------------------------------------
std::map<std::string, int32_t> JUIDialog::map_attributes_;
ATTRIBUTE_TYPE JUIDialog::attributes_[] =
{
{ "Title", ATTRIBUTE_PARAMETER_STRING }, };

JUIDialog::JUIDialog() :
        suspended_( false ), cancel_callback_( NULL ), dismiss_callback_( NULL ), activity_( NULL )
{
    map_attribute_parameters.clear();
}

JUIDialog::JUIDialog( ANativeActivity* activity ) :
        suspended_( false ), cancel_callback_( NULL ), dismiss_callback_( NULL )
{
    Init( activity );
}
...
void JUIDialog::CreateDialog()
{
    ...
    //Create dialog
    jmethodID mid = env->GetMethodID( JUIWindow::GetInstance()->GetHelperClass(), "createDialog",
            "(Landroid/app/NativeActivity;)Ljava/lang/Object;" );
    jobject obj = env->CallObjectMethod( JUIWindow::GetInstance()->GetHelperClassInstance(), mid, activity_->clazz );
    if( obj == NULL )
    {
    ...
    ndk_helper::JNIHelper::GetInstance()->CallVoidMethod( obj_, "setID", "(I)V", (int32_t) this );
    helper.Unlock();
}
...
void JUIDialog::Close()
{
    if( obj_ )
    {
        ...
        ndk_helper::JNIHelper::GetInstance()->CallVoidMethod( obj_, "dismiss", "()V" );
...
{
    ndk_helper::JNIHelper::GetInstance()->CallVoidMethod( obj_, "addView", "(Landroid/view/View;)V",
            view->GetJobject() );
        ...
        //Creating dialog asynchronous to avoid a crash inside a framework
            ...
            while( itBegin != itEnd )
            {
                ...
                ndk_helper::JNIHelper::GetInstance()->CallVoidMethod(
                        obj_, "addView", "(Landroid/view/View;)V",
                        (*itBegin)->GetJobject() );
...
void JUIDialog::DispatchEvent( const int32_t message,
...
bool JUIDialog::SetCallback( const int32_t message,
...
void JUIDialog::RestoreParameters( std::map<std::string, int32_t>& map )
{
    auto it = map_attribute_parameters.begin();
    auto itEnd = map_attribute_parameters.end();
    while( it != itEnd )
...
//JUIAlert dialog
//---------------------------------------------------------------------------
std::map<std::string, int32_t> JUIAlertDialog::map_attributes_;
ATTRIBUTE_TYPE JUIAlertDialog::attributes_[] =
{
...
JUIAlertDialog::JUIAlertDialog( ANativeActivity* activity ) :
        JUIDialog(), callback_positive_( NULL ), callback_negative_( NULL ), callback_neutral_( NULL )
{
    Init( activity );
}
...
//Add base class's map
        map_attributes_.insert( JUIDialog::map_attributes_.begin(), JUIDialog::map_attributes_.end() );
...
//Create dialog
    jmethodID mid = env->GetMethodID( JUIWindow::GetInstance()->GetHelperClass(), "createAlertDialog",
            "(Landroid/app/NativeActivity;)Ljava/lang/Object;" );
    jobject obj = env->CallObjectMethod( JUIWindow::GetInstance()->GetHelperClassInstance(), mid, activity_->clazz );
    if( obj == NULL )
    {
...
void JUIAlertDialog::DispatchEvent( const int32_t message,
        const int32_t param1,
        const int32_t param2 )
{
    switch( message )
...
bool JUIAlertDialog::SetButton( const int32_t button,
        const char* message,
